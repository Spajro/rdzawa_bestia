...
fn load_list<const S: usize>(value: &Value) -> [f32; S] {
    let mut result: [f32; S] = [0.0; S];
...
fn load_list_of_lists<const X: usize, const Y: usize>(value: &Value) -> Vec<[f32; X]> {
    let mut result: Vec<[f32; X]> = Vec::new();
    if let Value::List(list) = value {
        for i in 0..Y
        {
...
pub fn load_state<
    const X0: usize,
    const X1: usize,
    const X2: usize>(path: &String) -> Option<(Vec<[f32; X0]>, [f32; X1], Vec<[f32; X1]>, [f32; X2], Vec<[f32; X2]>, [f32; 1])> {
...
fn test() {
    ...
    let deserialized: BTreeMap<HashableValue, Value> = serde_pickle::from_slice(&*pickle, Default::default()).unwrap();
    let list: [f32; 256] = load_list(deserialized.get(&HashableValue::String(String::from("l1b"))).unwrap());
    let listlist: Vec<[f32; 2 * 40960]> = load_list_of_lists::<81920, 256>(deserialized.get(&HashableValue::String(String::from("l1w"))).unwrap());
}